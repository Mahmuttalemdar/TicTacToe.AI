...
hdiutil convert "tictactoe.dmg" -format UDRW -o "tictactoe_rw.dmg"

echo "Mount it and save the device"
DEVICE=$(hdiutil attach -readwrite -noverify "tictactoe_rw.dmg" | egrep '^/dev/' | sed 1q | awk '{print $1}')
sleep 5

echo "Create the sysmbolic link to application folder"
PATH_AT_VOLUME="/Volumes/tictactoe/"
CURRENT_PATH="$(pwd)"
cd "${PATH_AT_VOLUME}"
ln -s /Applications
cd "${CURRENT_PATH}"
...
#    tell disk "/Volumes/src:tictactoe"
#        open
...
#            set position of item "tictactoe.app" of container window to {160, 325}
...
hdiutil convert "tictactoe_rw.dmg" -format UDZO -o "tictactoe.dmg"
rm -f "tictactoe_rw.dmg"
